...
exports.listPizzas = function (req,res){
    Pizza.findAll({ attributes: ['id','name', 'price','image','desc']} )
        .then(data => {
...
exports.createPizza = async function(req,res){
    let pizza = Pizza.build({ name: req.body.name,price:req.body.price,image:req.body.image,desc:req.body.desc })
    // save object in DB
    await pizza.save()
        .then(data => {
            res.json(data);
